  ...
  class << self
    ...
    def inheritance(model, options = {})
      ...
      models.sort { |a, b| a.name <=> b.name }.each do |model|
        graph.node model.name, href: "https://forge.ekylibre.com/projects/serious/repository/revisions/master/entry/app/models/#{model.name.underscore}.rb"
      end
...
task diagrams: ['diagrams:all', 'diagrams:relational', 'diagrams:inheritance']
