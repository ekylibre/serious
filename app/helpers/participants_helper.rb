module ParticipantsHelper
  ...
  def participant_logo(participant)
    if participant.logo.file?
      image_tag(participant.logo.url(:identity))
    else
      content_tag(:div, participant.abbreviation, class: 'participant-abbreviation', style: "background-color: ##{participant.color}")
    end
  end
